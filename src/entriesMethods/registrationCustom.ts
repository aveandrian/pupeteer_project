...
export class Reg{
    static async registration(page:Page, methodId:string, i:number, creds:any, selectors:any){ 
        ...
        let telegramField;
        try{
            telegramField = await page.waitForSelector(methodId + selectors.telegramSelector, {visible:true});
        }catch(e){
            console.log("Telegram field not found");
        }
        if(telegramField!=null){
            await Utils.clear(page,  methodId + selectors.telegramSelector)
            await page.type(methodId + selectors.telegramSelector, creds.telegramsList[i]);
        }
