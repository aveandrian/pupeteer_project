...
const refLink = 'https://wn.nr/wD6n2P';
...
(async () => {
    ...
    for (let i=0; i<ids.length; i++){
        ...
        if(classesRegs.length > 1){
            ...
            await Reg.registration(page, methodsId[5], i, creds, selectors); 
        }
        ...
        for(let j=0;j<methodsId.length;j++){
            ...
            if(!classes[0].includes('completed-entry-method')){ 
                if(methods[j] == "twitterFollow"){
                    try{
                        await follow.twitter(page, browser, methodsId[j]);
                    }catch(e){}
                }
                    ...
                    try{
                        await follow.telegram(page, methodsId[j], j,creds);
                    }catch(e){}
                }
                    ...
                    try{
                        await Inputs.wallet(page, browser, methodsId[j], j,creds);
                    }catch(e){}
                    ...
                    try{
                        await Inputs.telegram(page, browser, methodsId[j], j, creds);
                }catch(e){}
                }else if(methods[j] == "emailInput"){
                    try{
                        await Inputs.email(page, browser, methodsId[j], j, creds);
                    }catch(e){}
                }
