...
const refLink = 'https://wn.nr/ZQzT4v';
...
const selectors = {
    ...
    walletSelector: " textarea[name='your_bep20_wallet_address']",
...
(async () => {
    ...
    for (let i=0; i<ids.length; i++){
        ...
        // await Utils.blockingWait(10);

        var methodsId:string[] = [];
        try{
            methodsId = await page.evaluate(
                () => Array.from(
        ...
        var classesRegs:string[] = [];
        try{
                 classesRegs = await page.evaluate(
                () => Array.from(
